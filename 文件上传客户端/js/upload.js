...
;(function () {
  ...
  // console.log(upload_button_select.classList, upload_button_upload.classList) // 样式类数组 ['upload_button', 'select']
    ...
    return new Promise(resolve => {
      ...
      fileReader.onload = ev => {
        let buffer = ev.target.result,
          ...
          HASH,
          suffix
        spark.append(buffer) // 附加一个字符串
        HASH = spark.end() // 完成 md5 的计算，返回十六进制结果。
...
;(function () {
  ...
  upload_button_upload.addEventListener('click', async function () {
    ...
    _files = _files.map(item => {
      let fm = new FormData(),
        curLi = upload_list_arr.find(liBox => liBox.getAttribute('key') === item.key), // 返回数组中满足提供的测试函数的第一个元素的值
        curSpan = curLi ? curLi.querySelector('span:nth-last-child(1)') : null // 从兄弟节点中从后往前匹配处于某些位置的元素
  ...
  upload_inp.addEventListener('change', async function () {
    _files = Array.from(upload_inp.files) // Array.from() 方法对一个类似数组或可迭代对象创建一个新的，浅拷贝的数组实例。
